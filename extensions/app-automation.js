...
export default function appAutomationExtension(pi) {
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      linkSort: Type.Optional(Type.String({ description: "Optional sort for per-app overview links: newest, oldest, freshest, stalest, app, kind, host, source, or from." })),
      includeExternal: Type.Optional(Type.Boolean({ description: "Load JSON app configs from APP_AUTOMATION_CONFIG_DIR. Defaults to true." })),
    }),
    async execute(_toolCallId, params) {
      const catalog = await resolveCatalog(params);
  ...
  pi.registerTool({
    ...
    parameters: Type.Object({
      ...
      sort: Type.Optional(Type.String({ description: "Optional link sort: newest, oldest, freshest, stalest, app, kind, host, source, or from." })),
