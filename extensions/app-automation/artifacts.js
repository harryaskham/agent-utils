...
function normalizeLinkSort(sort) {
  ...
  return ["newest", "oldest", "freshest", "stalest", "app", "kind", "host", "source", "from", "time"].includes(value) ? value : null;
}
...
function compareSnapshotLinks(sort) {
  ...
  return (a, b) => {
    ...
    if (mode === "source") {
      return String(a.context?.source || "unknown").localeCompare(String(b.context?.source || "unknown"))
        || String(a.app || "").localeCompare(String(b.app || ""))
        || String(a.kind || "").localeCompare(String(b.kind || ""))
        || String(a.label || "").localeCompare(String(b.label || ""))
        || String(a.url || "").localeCompare(String(b.url || ""));
    ...
    if (mode === "from") {
      return String(a.context?.from || "unknown").localeCompare(String(b.context?.from || "unknown"))
        || String(a.app || "").localeCompare(String(b.app || ""))
        || String(a.kind || "").localeCompare(String(b.kind || ""))
        || String(a.label || "").localeCompare(String(b.label || ""))
        || String(a.url || "").localeCompare(String(b.url || ""));
    ...
    if (mode === "time") {
      return String(a.context?.time || "unknown").localeCompare(String(b.context?.time || "unknown"))
        || String(a.app || "").localeCompare(String(b.app || ""))
        || String(a.kind || "").localeCompare(String(b.kind || ""))
        || String(a.label || "").localeCompare(String(b.label || ""))
        || String(a.url || "").localeCompare(String(b.url || ""));
...
function summarizeLinkHosts(links = []) {
  const counts = {};
  for (const link of links) {
    const key = linkHost(link);
    counts[key] = (counts[key] || 0) + 1;
  }
  return counts;
}
...
function summarizeLinkFrom(links = []) {
  const counts = {};
  for (const link of links) {
    const key = String(link.context?.from || "unknown");
    counts[key] = (counts[key] || 0) + 1;
  }
  return counts;
}
...
export async function collectSnapshotLinks({ root, app, query, source, from, time, host, freshness, kind, sort, artifactLimit = 100, linkLimit = 100, maxBytes = 64_000, staleAfterMinutes = 60, now = new Date() } = {}) {
  ...
  for (const artifact of listed.artifacts.filter((entry) => entry.extension === ".json")) {
    ...
    for (const row of linkedRows(value)) {
      ...
      for (const url of urls) {
        ...
        if (!linkMatchesQuery(link, query) || !linkMatchesContext(link, { source, from, time }) || !linkMatchesHost(link, host) || !linkMatchesFreshness(link, freshness) || !linkMatchesKind(link, normalizedKind)) continue;
        links.push(link);
      }
    }
  }
  ...
  return {
    ...
    fromCounts: summarizeLinkFrom(limitedLinks),
    links: limitedLinks,
    truncated: links.length > limit,
  };
}

export function aggregateSnapshotLinkSummaries({ root, snapshotRoot, summaries = [], query, source, from, time, host, freshness, kind, sort } = {}) {
  const normalizedKind = normalizeLinkKind(kind);
  const normalizedSort = normalizeLinkSort(sort);
  ...
  return {
    ...
    fromCounts: summarizeLinkFrom(links),
...
function snapshotLinkFilterParts(summary = {}) {
  ...
  if (summary.host) filters.push(`host=${JSON.stringify(summary.host)}`);
  return filters;
}
...
export function renderSnapshotLinks(summary) {
  ...
  const fromCounts = summary.fromCounts || summarizeLinkFrom(summary.links);
  ...
  const fromCountsText = Object.entries(fromCounts).sort(([a], [b]) => a.localeCompare(b)).map(([from, count]) => `${JSON.stringify(from)}=${count}`).join(",");
  ...
  const lines = [`links total=${counts.total}${matchedText}${scannedText} fresh=${counts.fresh} stale=${counts.stale} unknown=${counts.unknown}${summary.sort ? ` sort=${summary.sort}` : ""}${renderSnapshotLinkHeaderFilters(summary)}${appCountsText ? ` apps=${appCountsText}` : ""}${kindCountsText ? ` kinds=${kindCountsText}` : ""}${hostCountsText ? ` hosts=${hostCountsText}` : ""}${sourceCountsText ? ` sources=${sourceCountsText}` : ""}${fromCountsText ? ` from=${fromCountsText}` : ""}`];
