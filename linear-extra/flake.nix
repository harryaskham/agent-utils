{
  description = "Linear comment-draft MCP server";
  ...
  outputs =
    ...
    let
      ...
      editablesFixOverlay = final: prev: {
        linear-extra-mcp = prev.linear-extra-mcp.overrideAttrs (old: {
    ...
    in
    {
      devShells = eachDefaultSystem (system:
        let
          ...
          venv = pythonSet.mkVirtualEnv "linear-extra-mcp" workspace.deps.all;
        in
        {
      ...
      packages = eachDefaultSystem (system:
        let
          ...
          base = mkApplication {
            inherit venv;
            package = pythonSet.linear-extra-mcp;
          };
        in lib.fix (self: {
          default = self.linear-extra-mcp;
          linear-extra-mcp = pkgs.symlinkJoin {
            name = "linear-extra-mcp";
            paths = [ base ];
          };
        })
      );
      ...
      apps = eachDefaultSystem (system: {
        ...
        linear-extra-mcp = self.apps.${system}.default;
      });
    };
}
