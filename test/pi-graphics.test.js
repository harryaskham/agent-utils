...
test("pi-graphics settings source maps minimal env", async () => {
  ...
  assert.match(source, /function relativeCellOffsetPx\(cells, axis/);
  assert.match(source, /const cursorHOffsetCells = -Math\.floor\(cursorColumns \/ 2\)/);
  assert.match(source, /const cursorVOffsetCells = -Math\.floor\(cursorRows \/ 2\)/);
  assert.match(source, /const cursorHOffset = relativeCellOffsetPx\(cursorHOffsetCells, "x", cell\)/);
  assert.match(source, /const cursorVOffset = relativeCellOffsetPx\(cursorVOffsetCells, "y", cell\)/);
  ...
  assert.match(source, /H\/V are pixel offsets/);
  assert.doesNotMatch(source, /H=-5,V=-2/);
  ...
  assert.match(source, /hOffset: relativeCellOffsetPx\(-safeCursorCol, "x", cell\)/);
  ...
  assert.match(source, /hOffset: relativeCellOffsetPx\(FOOTER_DIVIDER_WIDTH, "x", cell\)/);
  ...
  assert.match(source, /action === "cursor-doctor"/);
  assert.match(source, /\["doctor", "help", "why"\]\.includes/);
  assert.match(source, /cursorDoctorLines\(\)\.join\("\\n"\)/);
...
test("pi-graphics extension source is the slim graphics primitive layer", async () => {
  ...
  assert.match(source, /function shouldSkipGraphicsOptions/);
  ...
  assert.match(source, /const patchedSetHeader = function \(componentOrFactory, \.\.\.rest\)/);
  assert.match(source, /wrapRenderableFactory\(componentOrFactory, "header"\)/);
  assert.match(source, /const patchedSetEditorComponent = function \(componentOrFactory, \.\.\.rest\)/);
  ...
  assert.match(source, /const patchedSetWorkingIndicator = function \(config, \.\.\.rest\)/);
  assert.match(source, /const patchedSetHiddenThinkingLabel = function \(label, \.\.\.rest\)/);
  assert.match(source, /__piGraphicsPatchedSurface = true/);
  assert.match(source, /ui\.notify\?\.__piGraphicsPatchedSurface/);
  assert.match(source, /ui\.custom\?\.__piGraphicsPatchedSurface/);
  assert.match(source, /ui\.setHeader\?\.__piGraphicsPatchedSurface/);
  assert.match(source, /ui\.setEditorComponent\?\.__piGraphicsPatchedSurface/);
  assert.match(source, /ui\.setStatus\?\.__piGraphicsPatchedSurface/);
