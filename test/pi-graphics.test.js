...
test("pi-graphics settings source maps minimal env", async () => {
  ...
  assert.match(source, /piGraphicsPlaceholderPlacementId\(`editor-cursor-anchor-\$\{cell\.cellWidthPx\}x\$\{cell\.cellHeightPx\}-\$\{editorCursorAnchorSeq\}`\)/);
  ...
  assert.match(source, /const cursorColumns = 11/);
  assert.match(source, /const cursorRows = 5/);
  assert.match(source, /hOffset: -Math\.floor\(cursorColumns \/ 2\)/);
  assert.match(source, /vOffset: -Math\.floor\(cursorRows \/ 2\)/);
  assert.match(source, /const anchorLine = buildKittyUnicodePlaceholderLines\(\{/);
  ...
  assert.match(source, /Expected live centering: fresh transparent anchor placement each redraw, 11x5 visible art, offsets -5,-2, stale cleanup by placement id\./);
