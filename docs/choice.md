...
- Press **Space** from the choice list to start a local-VAD push-to-talk reply.
  ...
  capture and returns to the choice. While a choice is active, it is the sole
  owner of Space/Enter/Escape/Ctrl-C: the ordinary editor PTT shortcut is
  suppressed, and the final transcript resolves the choice instead of queuing a
  normal user message behind the waiting tool.
...
Outside choices, empty-editor Space and Ctrl-Space are Pi editor shortcuts rather
than raw terminal listeners. This keeps them inactive while MCP selectors,
settings dialogs, overlays, and other focused custom components use Space for
their own controls.
