...
* Box chrome is enabled by default in caco-compatible `unicode` mode unless
  ...
  `/gfx cursor clear` deletes only the current live visible cursor placement and
  resets anchor diagnostics, giving operators a scoped recovery path if a terminal
  keeps stale cursor art after layout drift. This lets the heat glow, directional
  trail, frame ticks, and anchor centering be inspected without typing at a precise
