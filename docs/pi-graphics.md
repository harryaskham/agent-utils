...
* Message and TUI box chrome has per-surface effects (`glass`, `aurora`,
  `scanline`, `circuit`, `sparkle`, `cloud`, `prism`, and `holo`) and caches both
  ...
  the cloudy thought effect, input/overlay surfaces get a lightweight prism-facet
  glass treatment, and header/footer/session surfaces get a sparse holographic
  laminate. If a box moves or resizes, stale relative
