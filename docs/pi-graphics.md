...
to suppress the bounded per-message truecolor rail while keeping other calm
chrome active. The compact ANSI `/pi-graphics-ambient-proof`
strip, user-theme sync, transcript chrome, and the OSC terminal-palette request are enabled by default as fallback
visual smoke tests; transcript chrome inserts a bounded truecolor deep-Nordic
rail after normal messages so the transcript visibly changes even if core Pi
