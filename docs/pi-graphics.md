...
* Message and TUI box chrome has per-surface effects (`glass`, `aurora`,
  `scanline`, `circuit`, `sparkle`, `cloud`, `prism`, `holo`, `lattice`, and
  `contour`) and caches both uploads and relative placements so ordinary rerenders
  ...
  surfaces get calm topographic contour bands, input/overlay surfaces get a
