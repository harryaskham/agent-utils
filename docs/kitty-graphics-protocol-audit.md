...
- **Relative placements:** the live cursor and similar overlays use a transparent virtual-placeholder anchor plus relative placements, matching the protocol's intended pattern. Kitty `H`/`V` relative-placement offsets are pixel offsets, so Pi converts desired terminal-cell offsets through the active cell metrics before centering multi-cell overlays such as the 11×5 cursor glow. Although Kitty specifies that relative placements never move the cursor, live Pi/tmux sessions keep `C=1` on relative placement commands as a defensive cursor-movement suppression guard; omitting it caused editor scroll/input-line replication in practice. Raw relative-placement APC must also stay out of rendered editor text and be emitted through the side-channel writer.
- **Animation:** the docs and implementation now treat terminal-driven APNG/native loops as unreliable in the live Pi/tmux path and prefer explicit current-frame control (`a=a,c=<frame>`) where repainting is confirmed.
