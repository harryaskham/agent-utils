...
- **Relative placements:** Kitty `H`/`V` relative-placement offsets are terminal-cell offsets; positive values move right/down and negative values move left/up from the parent's top-left cell. Relative placements remain valid for box chrome, footer backgrounds, editor row backgrounds, previews, and animated editor borders. The live cursor is the exception: repeated live Kitty/tmux attempts to center an 11×5 relative child on a transparent 1×1 cursor anchor either left the child's top-left at the cursor or made the cursor disappear. The live default cursor therefore uses a direct Unicode-placeholder PNG in the cursor cell, matching the trailing-workspace path that is known to land correctly in the editor text flow. Raw relative-placement APC must still stay out of rendered editor text and be emitted through the side-channel writer.
