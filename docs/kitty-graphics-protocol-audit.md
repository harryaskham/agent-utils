...
- **Relative placements:** the live cursor and similar overlays use a transparent virtual-placeholder anchor plus relative placements, matching the protocol's intended pattern. Relative placement commands intentionally omit `C` because Kitty specifies that relative placements never move the cursor, and live cursor relative commands are emitted inline immediately after the anchor placeholder so H/V offsets resolve against the physical placeholder cell.
