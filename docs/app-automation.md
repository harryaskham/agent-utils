...
2. **Refresh if stale** — on ms-dev setups, run `app_automation_msdev_cdp_refresh` to pull bounded/redacted Slack, Outlook, Teams, and Calendar observations from Windows Chrome via PowerShell/CDP when the normal local browser bridge is unavailable.
