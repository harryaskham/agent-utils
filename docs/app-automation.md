...
- `/tendril-app [doctor|overview|links [app|all] [fresh|stale|unknown] [kind:<kind>] [query]|staleness|refresh-staleness|bundle|open-bundle|stale-refresh|app action]` — operator/agent-facing command for quick diagnostics, work-app overview, snapshot links, snapshot freshness, refresh-action freshness, default bundle discovery, and app/action planning.
