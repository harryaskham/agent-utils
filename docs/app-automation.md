...
2. **Refresh if stale** — on ms-dev setups, run `app_automation_msdev_cdp_refresh` to pull bounded/redacted Slack, Outlook, Teams, and Calendar observations from Windows Chrome via PowerShell/CDP when the normal local browser bridge is unavailable. If `ms-dev` is unreachable, pass a short `sshConnectTimeoutSeconds` value for a fast failure while preserving the latest manifest and stale snapshot context.
