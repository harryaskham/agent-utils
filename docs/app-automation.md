...
2. **Orient on current state** — run `app_automation_overview` or `/tendril-app overview` to see apps, active refreshers, app-level freshness, standard refresh-action freshness, and recent snapshot digests. Add `includeLinks: true` or `/tendril-app overview links` when you also want a small set of actionable snapshot URLs; use `/tendril-app overview links fresh kind:events link-sort:newest link-limit:5 stale-after:1440` for larger daily event-link samples. Overview link sections aggregate matched/truncated counts across apps and preserve the selected sort label/order so a small sample is not mistaken for the full match set.
