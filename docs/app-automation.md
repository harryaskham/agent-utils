...
- `app_automation_overview` — summarize configured work apps, active refreshers, snapshot freshness, and latest snapshot digests for quick orientation.
...
- `app_automation_snapshots_list` / `app_automation_snapshots_digest` / `app_automation_snapshots_staleness` / `app_automation_snapshot_read` — list, summarize, freshness-check, and read persisted JSON/Markdown/text/HTML snapshot artifacts under the state root without ad-hoc filesystem access.
