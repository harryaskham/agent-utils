...
- Typed startup special forms across the Agent Utils-owned settings subtree: with `agentUtils.globalShellExpansion.enabled=true`, ordinary literals remain valid while `$envPresent`, `$envAbsent`, `$envBool`, `$envEq`, and `$boolCommand` objects resolve recursively in memory. Pi core/third-party settings remain untouched, commands are explicit and bounded, failures are false, and settings are never rewritten. See the [settings special forms guide](docs/settings-special-forms.md).
