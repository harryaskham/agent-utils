...
- Lightweight shared Cacophony identity via [`extensions/cacophony-runtime.js`](extensions/cacophony-runtime.js): managed sessions consume existing identity, while unmanaged tmux sessions can opt into asynchronous `caco agent register` with `agentUtils.cacophony.autoRegister`. Durable session receipts deduplicate reload/restart, visiting identity is shared without rewriting parent env/settings, and `DISABLE_PI_CACO=1` disables Agent Utils Cacophony integrations globally. [`extensions/cacophony-mcp.js`](extensions/cacophony-mcp.js) consumes the same identity and uses pi-mcp-adapter's runtime API to attach one transient `caco mcp stdio` server, with child-scoped identity, automatic keep-alive discovery, and shutdown disposal. See the [Cacophony runtime guide](docs/cacophony-runtime.md).
