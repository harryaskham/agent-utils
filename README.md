...
- [`slick`](slick/README.md), a read-only Rust Slack TUI built on Ratatui + kittui's Ratakittui chrome. `slick daemon` is a rate-limit-aware collector that fills the stalest per-surface cache gaps and serves bearer-authenticated snapshots/SSE. Plain `slick` and `slick client` are the same smart client: they merge local cache plus daemon SSE by revision, retain remote snapshots for warm starts, and can acquire one per-host lease for read-only embedded fallback after a real source outage. Feed history seeds immediately from disk rather than visibly replaying on launch. The Slick subflake and root flake expose NixOS systemd, nix-darwin launchd, and Nix-on-Droid supervisord modules.
