...
- `/update` plus the `pi_self_update` native Pi tool via [`extensions/pi-self-update.js`](extensions/pi-self-update.js). `/update` runs `pi update` and reloads the Pi runtime afterwards; use `/update --no-reload` to skip the reload. The tool runs `pi update` itself and queues `/reload` as a follow-up so agents can update packages when newly landed changes need to appear in the current session.
