...
- `/effort [status|off|minimal|low|medium|high|xhigh|adaptive]` and `/fast [on|off]` via [`extensions/effort.js`](extensions/effort.js), shortcuts for Pi's current/default thinking effort setting without opening `/settings`. Unsupported values are rejected with the accepted list, and model-specific clamping is reported after setting. For reasoning-capable models (`reasoning: true` in model settings), `/effort adaptive` rewrites legacy `thinking.type=enabled` payloads to `thinking.type=adaptive` plus `output_config.effort` using the model's `thinkingLevelMap`; `/fast` toggles the same adaptive path with low effort for models that support it.
...
- `tendril_capture` — native Pi tool equivalent to `/tendril window|display`, capturing a target by id or unique name/title/app substring and returning PNG image content directly to the model.
- `tendril_describe` — native Pi tool equivalent to the model-facing half of `/tendril describe`, returning the captured PNG plus an objective visual-description prompt so the calling model can inspect the image directly.
- `tendril_stream` — native Pi tool equivalent to `/tendril stream start|status|stop`; started streams queue low-resolution follow-up screenshot messages for the model.
