...
- [`slick`](slick/README.md), a read-only Rust Slack TUI built on Ratatui + kittui's Ratakittui chrome. `slick daemon` is a rate-limit-aware collector that fills the stalest per-surface cache gaps and serves bearer-authenticated full/partial snapshots and SSE. Plain `slick` and `slick client` merge local cache plus daemon SSE by revision, retain remote snapshots for warm starts, and use one per-host lease for read-only fallback. Terse `slick feed`, `activity`, `dm`, `channel`, and `files` commands share implementations with `slick mcp stdio` tools through mcp-cli; `--json` emits stable compact envelopes and Canvas Markdown. Feed history seeds immediately from disk rather than visibly replaying on launch. The Slick subflake and root flake expose NixOS systemd, nix-darwin launchd, and Nix-on-Droid supervisord modules.
