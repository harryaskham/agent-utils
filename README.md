...
- Machine-global daemonless TTS playback coordination (`/tts-queue`, `tts_queue_status`, `tts_queue_configure`, and `tts_queue_control`) via [`extensions/tts-queue.js`](extensions/tts-queue.js), with file-backed queuing, bounded parallelism, early overlap, skip/next/clear controls, and stale-lock recovery. See [`docs/tts-queue.md`](docs/tts-queue.md).
