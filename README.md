...
- `tendril_pi_stream` — native Pi tool equivalent to `/tendril stream start|status|stop`; started streams queue low-resolution follow-up screenshot messages for the model by default, update the kitty terminal preview with a stable stream image id when preview is enabled, and the first frame includes current `tendril list` target context. Use `pathOnly: true` or `includeList: false` to reduce payload/context.
