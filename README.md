...
- `/effort [status|off|minimal|low|medium|high|xhigh|adaptive]` and `/fast [on|off]` via [`extensions/effort.js`](extensions/effort.js), shortcuts for model thinking and configured fast-model variants. Unsupported effort values are rejected with the accepted list, and model-specific clamping is reported after setting. For reasoning-capable models (`reasoning: true` in model settings), `/effort adaptive` rewrites legacy `thinking.type=enabled` payloads to `thinking.type=adaptive` plus `output_config.effort` using the model's `thinkingLevelMap` and any model-declared supported output effort values. Models can also explicitly force the adaptive format for ordinary levels like `medium` with `compat.thinkingFormat: "adaptive"`; supported output-effort metadata only clamps adaptive requests and does not turn adaptive mode on by itself. `/fast` is intentionally dumb: it only switches between the current model id and a configured sibling with the same id plus or minus a `-fast` suffix; it does not lower reasoning effort, enable adaptive thinking, or otherwise mutate request payloads. Set `agentUtils.trueDefaults.thinkingLevel`, `trueDefaultThinkingLevel`, or `defaultThinkingLevel` to `adaptive` to make adaptive thinking the startup default; true-defaults persists that setting but does not pass `adaptive` through Pi's core `setThinkingLevel`. GitHub Copilot Opus 4.8 adaptive thinking is clamped to `medium` only when adaptive rewriting is explicitly active.
