...
- Machine-global daemonless `/tts` + `/narrate` playback coordination via [`extensions/tts-queue.js`](extensions/tts-queue.js), with `/tts-queue` controls, serial playback with a two-second default overlap, bounded optional parallelism, and stale-lock recovery. Agent-facing queue tools are opt-in. See [`docs/tts-queue.md`](docs/tts-queue.md).
