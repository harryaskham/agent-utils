...
- [`slick`](slick/README.md), a read-only Rust Slack TUI built on Ratatui + kittui's Ratakittui chrome. `slick daemon` is a rate-limit-aware sole Slack collector that fills the stalest per-surface cache gaps and serves bearer-authenticated snapshots/SSE; `slick --client` is structurally cache-only and can follow local atomic writes or a remote daemon. Feed history seeds immediately from disk rather than visibly replaying on launch. The Slick subflake and root flake expose NixOS systemd, nix-darwin launchd, and Nix-on-Droid supervisord modules.
