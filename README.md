...
- `/effort [status|off|minimal|low|medium|high|xhigh|adaptive]` and `/fast [on|off]` via [`extensions/effort.js`](extensions/effort.js), shortcuts for Pi core thinking controls and configured fast-model variants. `/effort` delegates every supported value, including `adaptive`, directly to Pi's thinking-level API and never rewrites provider request payloads; adaptive-thinking request semantics and model metadata are owned by Pi itself. `/fast` switches only between the current model id and a configured sibling with the same id plus or minus a `-fast` suffix; it does not lower reasoning effort, enable adaptive thinking, or otherwise mutate request payloads.
- `/m <provider/model>` via [`extensions/m.js`](extensions/m.js), an independent model switcher that always resolves against the **full** model registry and switches immediately, with Tab-completion over every available `provider/model` string. Pi's built-in `/model` command and Ctrl+P cycling are governed by the scoped-models feature, so when models are scoped (for example via `enabledModels`) `/model <arg>` only matches the scoped set and arbitrary models fall through to the selector UI. `/m` deliberately ignores that scope so you can jump straight to any model without opening the selector or toggling its `all` scope; it never touches `/model` or `/scoped-models`, which keep governing Ctrl+P cycling. Matching mirrors Pi core: canonical `provider/id`, then a `provider/id` split, then an unambiguous bare id. `/m` with no argument prints usage and the available-model count.
...
- `tendril_stream` — native Pi tool equivalent to `/tendril stream start|status|stop`; started streams queue low-resolution follow-up screenshot messages for the model by default, update the kitty terminal preview with a stable stream image id when preview is enabled, and the first frame includes current `tendril list` target context. Use `pathOnly: true` or `includeList: false` to reduce payload/context.
