...
- `/update` plus the `pi_self_update` native Pi tool via [`extensions/pi-self-update.js`](extensions/pi-self-update.js). `/update` runs `pi update` and reloads the Pi runtime on success; use `/update --no-reload` to skip the reload. The tool queues the same command as a follow-up so agents can update when newly landed package changes need to appear in the current session.
