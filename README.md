...
- `/shell [command|preset]` and the `interactive_shell` tool via [`extensions/interactive-shell.js`](extensions/interactive-shell.js) provide a full-terminal overlay with Ctrl+] detach. Linux uses util-linux `script`; Darwin uses the packaged stdio-to-PTY Python bridge because BSD `script` cannot operate with Node pipe stdin.
