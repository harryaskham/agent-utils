...
- `/tendril-app [doctor|overview|links [app|all] [fresh|stale|unknown|freshness:<state>] [kind:<kind>] [query]|staleness|refresh-staleness|bundle|open-bundle|stale-refresh|app action]` — quick command for diagnostics, app/action discovery, snapshot links, freshness, and bundle guidance from the Pi UI.
