...
- `/effort [status|off|minimal|low|medium|high|xhigh]` via [`extensions/effort.js`](extensions/effort.js), a shortcut for Pi's current/default thinking effort setting without opening `/settings`. Unsupported values are rejected with the accepted list, and model-specific clamping is reported after setting.
